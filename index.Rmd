...
```{r, include = F}
defOut <- knitr::knit_hooks$get("include_graphics")  # save the default plot hook 
knitr::knit_hooks$set(include_graphics = function(x, options) {  # set new plot hook ...
  x <- defOut(x, options)  # first apply the default hook
  if(!is.null(options$float_graphics)) {  # then, if option float_graphics is given ...
    # create the new opening string for the wrapfigure environment ...
    wf <- sprintf("\\begin{float_graphics}{%s}{%g\\textwidth}", options$float_graphics[[1]], options$float_graphics[[2]])
    x  <- gsub("\\begin{figure}", wf, x, fixed = T)  # and replace the default one with it.
    x  <- gsub("{figure}", "{float_graphics}", x, fixed = T)  # also replace the environment ending
  }
  return(x)
})
```
...
```{r, echo = FALSE, float_graphics = list("R", .7)}
